...
const TimePage: React.FC<PageProps> = ({ viewMode }) => {
  ...
  useEffect(() => {
    ...
    if ('connection' in navigator) {
        ...
        updateNetwork();
        // Network info doesn't fire standard events reliably in all browsers, 
        // but we can poll or listen if supported.
    }

    // 4. Real Memory (Chrome Only - performance.memory)
    if ((performance as any).memory) {
        const mem = (performance as any).memory;
        // Update stats based on usedJSHeapSize / jsHeapSizeLimit
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center animate-fade-in text-retro-ink overflow-hidden">
      ...
      <div className={`
          ...
          ${viewMode === ViewMode.CONFIG ? 'w-[40vh] h-[40vh] opacity-20 translate-x-[-25vw]' : 'w-[60vh] h-[60vh] opacity-100'}
      ...
      <div className={`
          absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-1000 ease-[cubic-bezier(0.25,1,0.5,1)]
          ${viewMode === ViewMode.CONFIG ? 'w-[35vh] h-[35vh] opacity-20 translate-x-[-25vw]' : 'w-[50vh] h-[50vh] opacity-80'}
      ...
      {/* Main Clock Card (Slides Left/Scales Down) */}
      <div className={`
      ...
      `}>
        ...
        <h1 className="text-[22vw] md:text-[12rem] font-serif leading-none tracking-tighter text-retro-green drop-shadow-lg select-none whitespace-nowrap z-20">
            {formatTime(time)}
        </h1>
        
        {/* Date Box (Moved Up) */}
        <div className="mt-4 md:mt-6 px-8 py-2 bg-retro-brown text-retro-paper text-lg md:text-2xl font-sans italic tracking-widest uppercase shadow-md relative whitespace-nowrap z-20">
            {/* Decors on label */}
        ...
        {/* Decorative Time Scale / Ruler (Moved Down) */}
        <div className="w-full h-8 mt-6 md:mt-8 relative flex justify-between items-start opacity-50">
             {[...Array(20)].map((_, i) => (
      ...
      {/* --- DASHBOARD WIDGETS (Slide in from Right) --- */}
      <div className={`
          absolute right-0 top-0 bottom-0 w-[50vw] md:w-[40vw] bg-retro-paper-dark/95 border-l-4 border-retro-gold
          flex flex-col p-4 md:p-8 transition-transform duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] z-20 shadow-2xl
          ${viewMode === ViewMode.CONFIG ? 'translate-x-0' : 'translate-x-full'}
      `}>
          <div className="h-full flex flex-col justify-center gap-6 md:gap-8 overflow-y-auto">
              <h3 className="font-serif font-bold text-retro-ink tracking-widest text-xl md:text-2xl border-b-2 border-retro-gold pb-2">SYS.STATUS</h3>
              
              {/* STATUS 1: POWER */}
              <div className="flex flex-col gap-2">
                  <div className="flex justify-between font-mono text-xs text-retro-gray tracking-widest">
                      <span>POWER_LEVEL</span>
                      <span>{stats.voltage.toFixed(1)}V</span>
                  </div>
                  <div className="w-full h-4 bg-retro-gray/20 border border-retro-gray/40 p-[2px]">
                      <div 
                  ...
                  <div className="text-right font-mono text-xs text-retro-gold flex flex-col items-end">
                      <span>{stats.battery}% // {stats.isCharging ? 'CHARGING' : 'DISCHARGING'}</span>
                      {!usingRealBattery && (
                          <span className="text-[9px] text-retro-red/80 mt-1 font-bold">⚠ SIMULATION MODE (API UNAVAILABLE)</span>
                      )}
                  </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                  ...
                  <div className="text-right font-mono text-xs text-retro-gold">SECURE CONNECTION</div>
              </div>
               ...
               <div className="flex flex-col gap-2">
                  ...
                  <div className="w-full h-2 bg-retro-gray/20 overflow-hidden flex gap-[2px]">
              ...
              <div className="mt-auto font-mono text-[10px] text-retro-gray/40 text-center tracking-[0.5em] animate-pulse">
                  /// NO ALERTS DETECTED ///
              </div>
          </div>
      </div>
